...
import { IoObject, Num, Str, BinOp, Message, NIL, Method, UserObject } from "./object"
import { Slot } from "./slot"
import { IF, PRINT, FUN } from "./builtin-functions"

const arithmeticBinOp = new Set(["+","-","*","/","%"])
const compareBinOp = new Set(["==","!=","<","<=",">",">="])
const specialBinOp = new Set(["=", ":=", ".", ";", ",","&&","||"])
...
function evalMessage(target:IoObject|undefined,	message:Message, e:Slot):IoObject{
	//console.log(`evalMessage(${target?.str()},${message.str()})`)
	if(target){
		if(target instanceof UserObject){
			return target.send(message.name, message.args?.map(x=>evalNode(x,e)), e)
		}else{
			return target.send(message.name, message.args?.map(x=>evalNode(x,e)), e)
		}
	}else{
		if(!message.args){  // as variable
...
function evalBinOp(bop: BinOp, e:Slot):IoObject{
	...
	if(arithmeticBinOp.has(bop.op)){
		return evalArithmeticOp(bop, e)
	}else if(compareBinOp.has(bop.op)){
		return evalCompareOp(bop, e)
	}else if(specialBinOp.has(bop.op)){
...
function evalAssign(flag:("define"|"update"), target:IoObject, value:IoObject, e:Slot):IoObject{
	...
	if(target instanceof BinOp && target.op === "."){
		...
		if(targetObject instanceof UserObject && target.rhs instanceof Message && !target.rhs.args){
...
function assignObjectSlot(targetObj:UserObject, varName:string, value:IoObject):IoObject{
	//console.log(`assignObjectSlot(${targetObj.str()},${varName},${value.str()})`)
	return targetObj.assignToObject(varName, value)
}

function assignLocalVariable(flag:("define"|"update"),varName:string, value:IoObject, e:Slot):IoObject|null{
