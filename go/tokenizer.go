...
func filter1(tokens []Token) []Token {
	...
	result = append(result, tokens[len(tokens)-1]) // last one
	return result
}
...
func IsLPar(t Token) bool {
	return t.Type == TokenLPar
}

func IsRPar(t Token) bool {
	return t.Type == TokenRPar
}

func IsBinOp(t Token) bool {
	return t.Type == TokenBinOp
}

func IsSym(t Token) bool {
	return t.Type == TokenSym
}
