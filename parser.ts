import { Token, TokenType, tokenize, isBinOp, isLPar, isRPar, isTerminator } from "./tokenizer"
import { IoObject, Num, Str, BinOp, Message, Apply } from "./object"
...
function parseFactor(reader:TokenReader):IoObject{
	...
	if(token.type === "num"){
		...
		if(reader.endOfToken()){
			...
			if(isRPar(reader.seeNext(2))){  // no argument
